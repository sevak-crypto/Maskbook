...
export async function swapQuote(request: SwapQuoteRequest) {
    ...
    if (request.slippagePercentage)
        params.set('slippagePercentage', new BigNumber(request.slippagePercentage).div(BIPS_BASE).toString())
    if (request.buyTokenPercentageFee)
        params.set('buyTokenPercentageFee', new BigNumber(request.buyTokenPercentageFee).div(100).toString())
