...
export const IdentityProviderMinds: SocialNetworkUI.CollectingCapabilities.IdentityResolveProvider = {
    async start(signal) {
...
async function query(): Promise<null | SocialNetworkUI.CollectingCapabilities.IdentityResolved> {
    ...
    db.close()
    return {
        identifier: new ProfileIdentifier(mindsBase.networkIdentifier, detail.username),
